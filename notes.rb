...
This simple application is designed to allow multiple users to access the same database to update their own records AND access the records of others.  A sample instance would be bitmaker labs.  Students would each enter their own contact info but also access the contact info of others in the group. 

There is no security layer assumed.  Each user can add, modify, or change any other user records.
...
2. As a user I should be prompted to add/delete/etc contacts from within the console
...
1. As a developer, I should be able to capture the inputs as required by the user
2. I should be able to store the inputs in a database 
3. 
...
3. I should store the data in a local instance (i.e. the data is not saved to a file)
...
* display all available instructions to the user
* recieve inputs from the user
* respond to user inputs with both prompts and actions (methods) as defined in the database class 
* send messages from the runner to the 'program' in order to execute the program
* handle exceptions
